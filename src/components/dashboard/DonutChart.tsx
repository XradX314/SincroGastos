...
export function DonutChart({ data, title = 'Gasto por categoría' }: DonutChartProps) {
  ...
  const filtered = activeCategory ? chartData.filter((d) => d.name === activeCategory) : chartData
    ...
    <Card>
      ...
      <div className="relative h-52">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className="text-base font-bold text-text1 dark:text-text1-dark">{formatARS(total)}</span>
        </div>
      </div>
        ...
        {chartData.map((entry) => (
          <button
            ...
            className={[
              ...
              activeCategory === entry.name ? 'bg-surface-2 dark:bg-surface-2dark' : 'hover:bg-surface-2 dark:hover:bg-surface-2dark',
              activeCategory && activeCategory !== entry.name ? 'opacity-40' : '',
            ].join(' ')}
          >
            <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: entry.color }} />
            <span className="text-xs text-text1 dark:text-text1-dark truncate flex-1">{entry.name}</span>
